...
output$hist <- renderSVG({
  reactive({
    ...
    if (nrow(data) == 0) {
      ...
      tryCatch(
        {
          result <- make_histogram(data, mu_data, FALSE, input$group, input$split, input$plot, input$bin, input$position, baseSize = input$baseSize, disable_size_check = input$disableSizeCheck)
...
output$hist_plotly <- renderPlotly({
  ...
  tryCatch(
    {
      p <- make_histogram(data, mu_data, FALSE, input$group, input$split, input$plot, input$bin, input$position, baseSize = input$baseSize, disable_size_check = input$disableSizeCheck)
