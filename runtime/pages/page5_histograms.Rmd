...
selectizeInput("dataType", "Data Type",
  choices = c("Gait Data" = "gait", "Simulation Data" = "simulation", "Raw Tracker Data" = "raw_tracker"),
...
for (data_type in names(cacheable_data_types)) {
  ...
  cache_managers[[data_type]] <- create_shiny_cache_manager(data_type, parallel = FALSE)
...
# Helper function to get cached data for a data type
get_cached_data <- function(data_type) {
...
currentData <- reactive({
  if (input$useSummarizedData) {
    ...
    if (input$dataType %in% names(cacheable_data_types)) {
      ...
      data <- get_cached_data(input$dataType)
...
for (data_type in names(cacheable_data_types)) {
  ...
  output[[status_id]] <- renderText({
    if (input$dataType == data_type) {
      ...
      data <- get_cached_data(data_type)
      if (nrow(data) > 0) {
