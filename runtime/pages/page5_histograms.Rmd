...
# Parallel loading control for cache managers
checkboxInput("enableParallelLoading", "Enable parallel data loading",
  value = TRUE
)
...
for (data_type in names(cacheable_data_types)) {
  # Create cache manager with default parallel loading (will be updated reactively)
  cache_managers[[data_type]] <- create_shiny_cache_manager(data_type, use_parallel = TRUE)
...
# Helper function to get cached data for a data type (only if already loaded)
get_cached_data_local <- function(data_type) {
  if (data_type %in% names(cache_managers)) {
    # Only return data if it's already been loaded (trigger > 0)
    trigger_value <- cache_managers[[data_type]]$trigger()
    if (trigger_value > 0 && data_type %in% names(cached_data_reactive)) {
      page_logger("DEBUG", "Getting already loaded data for", data_type)
      return(cached_data_reactive[[data_type]]())
    } else {
      page_logger("DEBUG", "No data loaded yet for", data_type, "(trigger =", trigger_value, ")")
    }
  }
...
currentData <- reactive({
  ...
  if (input$dataType == "summarized") {
    ...
    # For cacheable data types, only return data if it's already been loaded
    # Don't trigger automatic loading - only show what's already cached
    page_logger("DEBUG", "Checking for existing cached data for type:", input$dataType)

    # Check if we have any cached data without triggering new loads
    if (input$dataType %in% names(cached_data_reactive)) {
      # Only get data if it's already been loaded (trigger > 0)
      trigger_value <- cache_managers[[input$dataType]]$trigger()
      if (trigger_value > 0) {
        page_logger("DEBUG", "Data has been requested (trigger > 0), getting cached data")
        data <- cached_data_reactive[[input$dataType]]()
        ...
        page_logger("DEBUG", "No data request made yet (trigger = 0), returning empty structure")
      }
    }
...
# Observer for parallel loading checkbox - update parallel setting in existing cache managers
observeEvent(input$enableParallelLoading,
  {
    page_logger("DEBUG", "Parallel loading setting changed to:", input$enableParallelLoading)

    # Update parallel setting in existing cache managers
    use_parallel <- if (!is.null(input$enableParallelLoading)) input$enableParallelLoading else TRUE
    for (data_type in names(cacheable_data_types)) {
      cache_managers[[data_type]]$parallel_enabled(use_parallel)
    }

    page_logger("DEBUG", "Updated parallel loading setting to:", use_parallel, "for all cache managers")
  },
  ignoreNULL = TRUE
)
