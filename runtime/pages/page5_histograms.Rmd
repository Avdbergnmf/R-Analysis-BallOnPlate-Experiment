...
conditionalPanel(
  ...
  div(
    ...
    div(
      ...
      "Note: Simulation data is automatically loaded and cached when needed."
    )
  )
)
...
currentData <- reactive({
  if (input$useSummarizedData) {
    ...
    if (input$dataType == "simulation") {
      # Use new caching system for simulation data
      if (!is.null(input$filterParticipants) && !is.null(input$filterTrials)) {
        # Get simulation data using the new caching system
        sim_data <- get_cached_data(
          data_type = "simulation",
          participants = input$filterParticipants,
          trials = input$filterTrials,
          condition_filter = input$filterCondition
        )
...
# Note: Simulation data is now automatically loaded and cached using the new caching system

# Status display for simulation data
output$simulationStatus <- renderText({
  if (input$dataType == "simulation") {
    # Get current data using the new caching system
    data <- currentData()
    if (nrow(data) > 0) {
      sprintf(
      ...
      if (!is.null(input$filterParticipants) && !is.null(input$filterTrials)) {
        "Loading simulation data... (automatically cached)"
      } else {
        "Please select participants and trials to load simulation data."
      }
    }
  } else {
    ""
