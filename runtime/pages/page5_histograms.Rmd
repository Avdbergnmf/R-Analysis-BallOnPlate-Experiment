...
```{r, context="server"}
# Reactive data source based on data type selection - only compute when page is active
currentData <- create_conditional_reactive("Histograms", {
...
# Create conditional observers that only run when this page is active
create_conditional_observer("Histograms", {
...
output$simulationStatus <- renderText({
  # Only render when page is active
  if (!is_page_active("Histograms")) {
    return("")
  }
...
plotCount <- create_conditional_reactive("Histograms", {
...
plotHeight <- create_conditional_reactive("Histograms", {
  if (input$split == "None") {
...
output$hist <- renderSVG({ reactive({
    # Only render when page is active
    if (!is_page_active("Histograms")) {
      return(ggplot() + theme_void() + ggtitle("Page not active"))
    }
