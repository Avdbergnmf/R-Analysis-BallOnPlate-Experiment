...
page_logger <- create_module_logger("PAGE11-CORRELATIONS")
page_logger("DEBUG", "Initializing correlations page options")
...
apply_correlation_transforms <- function(data, x_var, y_var, x_transform, y_transform) {
  page_logger("DEBUG", "Applying transformations - X:", x_transform, "Y:", y_transform)
  # Apply transformation to X variable
  if (x_transform != "none" && x_var %in% colnames(data)) {
    data <- stats$transform_dependent_variable(data, x_var, x_transform)
    if (is.null(data)) {
      return(NULL)
    }
  }
  ...
  if (y_transform != "none" && y_var %in% colnames(data)) {
    data <- stats$transform_dependent_variable(data, y_var, y_transform)
    if (is.null(data)) {
      return(NULL)
    }
  }
...
get_correlation_data <- reactive({
  page_logger("DEBUG", "Getting correlation data - summarized:", input$getSummarizedData, "averaged:", input$averageData)
  if (input$getSummarizedData) {
    data <- get_mu_dyn_long()
    if (input$averageData) {
      page_logger("DEBUG", "Summarizing data across conditions")
...
output$homoscedasticity_plot <- renderSVG({
  reactive({
    page_logger("DEBUG", "Rendering homoscedasticity plot")
    dt <- get_correlation_data()

    # Apply transformations
    dt <- apply_correlation_transforms(dt, input$xVar, input$yVar, input$xVarTransform, input$yVarTransform)
    if (is.null(dt)) {
      page_logger("WARN", "Transformation failed for homoscedasticity plot")
      plot.new()
...
output$correlation_plot_svg <- renderSVG({
  reactive({
    ...
    plotting$plot_correlation_stats(dt, input$xVar, input$yVar, input$par, input$baseSize, input$heatmap, input$heatmap_bins)
  })
})
