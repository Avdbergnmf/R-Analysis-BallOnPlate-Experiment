...
output$correlation_text <- renderPrint({
  ...
  tryCatch({
    if (method == "bayes") {
      ...
      cat("r:", correlation$estimate, " p:", correlation$p.value, " CI:",
...
output$correlation_plot_svg <- renderSVG({ reactive({
  dt <- get_correlation_data()
...
output$correlation_plotly <- renderPlotly({
  ...
  hover_text <- paste0(
    ...
    "<br>", input$xVar, ": ", signif(dt[[input$xVar]], 4),
    "<br>", input$yVar, ": ", signif(dt[[input$yVar]], 4)
  )
  plotly::plot_ly(
    data = dt,
