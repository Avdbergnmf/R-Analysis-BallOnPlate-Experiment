...
page_logger <- create_module_logger("PAGE8-QUESTIONNAIRES")

output$q <- renderSVG({
  reactive({
    page_logger("DEBUG", "Rendering questionnaire plot for:", input$plotQ)

    # Get filtered questionnaire data
    data <- questionnaire$filter_questionnaire_results(filteredQResults(), input$plotQ)
    page_logger("DEBUG", "Filtered questionnaire data retrieved, rows:", nrow(data))
