...
page_logger <- create_module_logger("PAGE13-TABLE")
page_logger("DEBUG", "Initializing data table page options")
...
observe({
  page_logger("DEBUG", "Initializing participant and trial choices for table")
  # Get participants and trials from global environment
  participant_choices <- sort(participants)
  trial_choices <- sort(allTrials)
...
tableToUse_13 <- reactive({
  page_logger("DEBUG", "Preparing table data - summarized:", input$avgTable_13, "filtered:", input$enableFilter_13)
  ...
  page_logger("DEBUG", "Table data prepared, rows:", nrow(base_data))
  return(base_data)
})

output$full_results_table_interactive_13 <- DT::renderDataTable({
  page_logger("DEBUG", "Rendering interactive data table")
  dt <- tableToUse_13()
  DT::datatable(
    dt,
    options = list(
