...
observeEvent(event_data("plotly_relayout", source = "manual_outlier_plot"), {
  ...
  handle_outlier_toggle <- function(clicked_time, current_participant, current_trial, trial_data, mode, 
    ...
    closest_step <- outliers$find_closest_strike(clicked_time, current_participant, current_trial, trial_data, outlier_threshold)
...
observeEvent(event_data("plotly_click", source = "manual_outlier_plot"), {
  ...
  if (!is.null(click_data) && is.list(click_data) && !is.null(click_data$x)) {
    ...
    closest_step <- outliers$find_closest_strike(clicked_time, current_participant, current_trial, trial_data, input$outlier_threshold)
...
observeEvent(input$apply_to_workspace, {
  tryCatch({
    ...
    hs_matches  <- outliers$find_heel_strikes(current_hs_outliers, gait_dt, threshold)
    ...
    step_matches <- outliers$find_heel_strikes(current_step_outliers, gait_dt, threshold)
...
observeEvent(input$confirm_apply_outliers, {
  tryCatch({
    ...
    allGaitParams <<- outliers$apply_outlier_processing(
      allGaitParams,
      current_hs_outliers,
