...
actionButton("load_default_outliers", "📂 Load Default Outlier Files", 
...
downloadButton("export_heelstrike_outliers", "📤 Export Heel Strikes", class = "btn-secondary")

fileInput("importStepOutliers", "Import Step Outliers",
  accept = c("text/csv", "text/comma-separated-values,text/plain", ".csv"))
downloadButton("export_step_outliers", "📤 Export Steps", class = "btn-secondary")
```
...
actionButton("manual_backup", "💾 Manual Backup Both Tables", 
...
observeEvent(input$load_default_outliers, {
  tryCatch({
    ...
    if (length(errors) > 0) {
      ...
      success_msg <- paste0(
        "✅ Default outlier files loaded successfully!\n",
...
observeEvent(input$manual_backup, {
  ...
  backup_message <- paste0(
    ...
    "• Heel Strike Outliers: ", num_heelstrike, " entries\n",
    "  → Will save to: ", file.path(dataExtraFolder, falseHeelStrikesFile), "\n\n",
    "• Step Outliers: ", num_step, " entries\n",
    "  → Will save to: ", file.path(dataExtraFolder, outliersFile), "\n\n",
    "⚠️ This will permanently overwrite existing files!\n\n",
  ...
  showModal(modalDialog(
    ...
    HTML(paste0(
      ...
      "❌ <strong>Heel Strike Outliers:</strong> ", num_heelstrike, " entries<br>",
      "   → File: <code>", file.path(dataExtraFolder, falseHeelStrikesFile), "</code><br><br>",
      "◐ <strong>Step Outliers:</strong> ", num_step, " entries<br>", 
      "   → File: <code>", file.path(dataExtraFolder, outliersFile), "</code><br><br>",
      "<span style='color: #d9534f;'><strong>⚠️ Warning:</strong> This will permanently overwrite existing files!</span>",
      "</div>"
    )),
    footer = tagList(
      modalButton("❌ Cancel"),
      actionButton("confirm_manual_backup", "✅ Confirm Backup", 
...
observeEvent(input$confirm_manual_backup, {
  ...
  tryCatch({
    ...
    heel_save_path <- file.path(dataExtraFolder, falseHeelStrikesFile)
    ...
    step_save_path <- file.path(dataExtraFolder, outliersFile)
    ...
    showNotification(
      paste("✅ Manual backup successful!", 
    ...
    showNotification(paste("❌ Backup failed:", e$message), type = "error", duration = 10)
  })
})
...
save_outlier_tables <- function(overwrite_mode = FALSE){
  ...
  write.csv(heelstrike_outliers(), path_hs, row.names = FALSE)
  write.csv(step_outliers(),      path_step, row.names = FALSE)

  # Update timer
  last_save_time(Sys.time())

  list(hs_path = path_hs, step_path = path_step)
}
...
observeEvent(input$manual_backup, {
  ...
  showNotification(
    paste("💾 Manual backup saved to", basename(res_paths$hs_path), "&", basename(res_paths$step_path)),
    type = "message", duration = 5
  )
})
...
observe({
  ...
  tryCatch({
    showNotification(
      paste("✅ Backup successful to", basename(res_paths$hs_path), "&", basename(res_paths$step_path)),
      type = "message", duration = 5
    )
    
    # Reset autosave timer by updating last_save_time (already done in save function)
  }, error = function(e) {
...
observe({
  ...
  isolate({
    last_click_info(list(participant = input$participant,
                         trial = input$trialNum,
