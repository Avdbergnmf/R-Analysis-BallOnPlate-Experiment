...
observeEvent(input$save_changes, {
  ...
  tryCatch({
    # Save to current working files (overwrite mode)
    result <- save_outlier_tables(overwrite_mode = TRUE)
    ...
    showNotification(
      HTML(paste0(
        "✅ <strong>Changes saved successfully!</strong><br>",
...
observeEvent(input$confirm_save_as, {
  ...
  tryCatch({
    ...
    result <- save_outlier_tables(overwrite_mode = FALSE, 
    ...
    showNotification(
      HTML(paste0(
        "✅ <strong>Saved as new files!</strong><br>",
        "📁 <code>", hs_filename, "</code><br>",
        "📁 <code>", step_filename, "</code><br><br>",
        hs_message, "<br>",
        step_message, "<br><br>",
        "💡 These are now your current working files"
      )),
      type = "message", duration = 8
    )
    
  }, error = function(e) {
    showNotification(paste("❌ Save As failed:", e$message), type = "error", duration = 8)
  })
})
...
observe({
  ...
  # Skip if interval not yet elapsed since last save
  elapsed <- as.numeric(difftime(Sys.time(), last_save_time(), units="secs"))
  if (elapsed < input$autosave_interval*60) {
    return()
  }
  
  # Skip if no data to save
  if (nrow(hs_outliers_data) == 0 && nrow(step_outliers_data) == 0) {
    return()
  }
  
  tryCatch({
    res_paths <- save_outlier_tables(overwrite_mode)
    
    # If we saved successfully and it was overwrite mode, mark as no changes
    if (overwrite_mode) {
      current_hash <- calculate_current_hash()
      last_saved_hs_hash(current_hash$hs)
      last_saved_step_hash(current_hash$step)
    ...
    # Create concise change summary for autosave
    hs_changes <- res_paths$hs_changes
    step_changes <- res_paths$step_changes
    
    # Show concise autosave notification
    change_summary <- paste0(
      "HS: ", hs_changes$new_count, " (", 
      if (hs_changes$net_change > 0) paste0("+", hs_changes$net_change) else hs_changes$net_change,
      "), Step: ", step_changes$new_count, " (",
      if (step_changes$net_change > 0) paste0("+", step_changes$net_change) else step_changes$net_change,
      ")"
    )
    
    showNotification(
      paste("✅ Auto-save:", change_summary, "to", basename(res_paths$hs_path), "&", basename(res_paths$step_path)),
      type = "message", duration = 3
    )
...
output$overwrite_warning <- renderUI({
  if (input$autosave_overwrite) {
    ...
    div(style = "font-size: 11px; color: #d63384; margin-top: -5px; margin-bottom: 10px; background-color: #fff3cd; padding: 5px; border-radius: 3px; border-left: 3px solid #ffc107;",
        paste0("⚠️ WARNING: Will overwrite ", current_hs_file, " and ", current_step_file),
        br(),
    ...
    div(style = "font-size: 11px; color: #666; margin-top: -5px; margin-bottom: 10px;",
        "✓ Safe mode: Creates timestamped backup files")
  }
})
```