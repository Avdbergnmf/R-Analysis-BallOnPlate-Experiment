...
observeEvent(event_data("plotly_click"), {
  ...
  if (!is.null(click_data) && is.list(click_data) && !is.null(click_data$x)) {
    ...
    if (!is.null(closest_step)) {
      ...
      # Determine action based on the selected mode and current state
      mode_to_use <- input$outlier_mode  # "heel" or "step"
      
      if ((mode_to_use == "heel" && is_hs_outlier) ||
          (mode_to_use == "step" && is_step_outlier)) {
        # Heel-strike already has the selected mode → toggle OFF (remove)
        mode_to_use <- "remove"
        action_type <- "removed"
      } else {
        # Heel-strike not yet in selected mode → toggle ON (add)
        action_type <- "added"
      }
