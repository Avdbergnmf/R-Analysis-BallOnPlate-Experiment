...
div(style = "margin-top: 8px; padding: 6px; background-color: #f0f8ff; border-radius: 4px; border-left: 3px solid #007bff; font-size: 11px;",
  div("💡 Click to cycle: none → selected → alternate → none")
)
...
div(style = "font-size: 11px; color: #666; margin-top: -5px; margin-bottom: 10px;",
    paste0("✓ Checked: Overwrites ", falseHeelStrikesFile, " and ", outliersFile),
    br(),
...
observeEvent(event_data("plotly_relayout"), {
  ...
  # Update outliers based on plot clicks (cycling based on current state)
observeEvent(event_data("plotly_click"), {
  ...
  if (!is.null(click_data) && is.list(click_data) && !is.null(click_data$x)) {
    ...
    if (!is.null(closest_step)) {
      ...
      if (is_hs_outlier) {
        # Currently option 1 (heel strike outlier) -> cycle to option 2 (step outlier)
        mode_to_use <- "step"
        action_type <- "switched"
      } else if (is_step_outlier) {
        # Currently option 2 (step outlier) -> cycle to none (remove)
        mode_to_use <- "remove"
        action_type <- "removed"
      } else {
        # Currently none -> cycle to option 1 (selected mode)
...
observe({
  ...
  if (overwrite_mode) {
    ...
    hs_filename <- falseHeelStrikesFile
    step_filename <- outliersFile
