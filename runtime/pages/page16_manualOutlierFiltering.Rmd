...
output$manualOutlierPlot <- renderPlotly({
  ...
  if (nrow(current_outliers_data) > 0 && nrow(current_data) > 0) {
    ...
    time_diffs <- outer(current_data$time, current_outliers_data$time, 
...
observeEvent(input$overwrite_outliers, {
  ...
  if (nrow(outliers_data) > 0) {
    ...
    gait_dt <- data.table(
      ...
      time_rounded = round(allGaitParams$time / time_precision) * time_precision,
    ...
    outliers_dt <- data.table(
      ...
      time_rounded = round(outliers_data$time / time_precision) * time_precision
    )
    ...
    if (length(matched_rows) > 0) {
      allGaitParams$outlierSteps[matched_rows] <<- TRUE
    }
  }
