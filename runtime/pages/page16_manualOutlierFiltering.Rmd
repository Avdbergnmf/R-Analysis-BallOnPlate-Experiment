...
fluidRow(
  column(
    6,
    actionButton("load_manual_outliers", "📂 Load Outlier Files",
...
page_logger <- create_module_logger("PAGE16-OUTLIER-FILTER")
...
current_heelstrike_outliers <- reactive({
  page_logger("DEBUG", "Filtering heelstrike outliers for participant:", input$participant_16, "trial:", input$trialNum_16)
  heelstrike_outliers() %>%
    dplyr::filter(participant == input$participant_16 & trialNum == as.numeric(input$trialNum_16))
})

current_step_outliers <- reactive({
  page_logger("DEBUG", "Filtering step outliers for participant:", input$participant_16, "trial:", input$trialNum_16)
  step_outliers() %>%
    dplyr::filter(participant == input$participant_16 & trialNum == as.numeric(input$trialNum_16))
})
...
handle_outlier_toggle <- function(clicked_time, current_participant, current_trial, trial_data, mode,
  ...
  closest_step <- gait$find_closest_strike(clicked_time, current_participant, current_trial, trial_data, outlier_threshold)
...
observeEvent(event_data("plotly_click", source = "manual_outlier_plot"), {
  ...
  if (!is.null(click_data) && is.list(click_data) && !is.null(click_data$x)) {
    ...
    closest_step <- gait$find_closest_strike(clicked_time, current_participant, current_trial, trial_data, input$outlier_threshold)
...
observeEvent(input$nav_prev_special, {
  ...
  if (nrow(prev_specials) > 0) {
    ...
    showNotification(paste(
      "Navigated to previous", gsub("_", " ", target_type), "at time", round(target_time, 3),
      "(≥", min_threshold, "s away)"
    ), type = "message")
  } else {
...
observeEvent(input$nav_next_special, {
  ...
  if (nrow(next_specials) > 0) {
    ...
    navigate_to_time(target_time, input$xplot_16, input$yplot_16)
    showNotification(paste(
      "Navigated to next", gsub("_", " ", target_type), "at time", round(target_time, 3),
      "(≥", min_threshold, "s away)"
    ), type = "message")
  } else {
...
output$nav_status <- renderText({
  ...
  if (nrow(special_data) == 0) {
    ...
    paste(
      "Found", nrow(special_data), "special heel strikes:",
...
output$manualOutlierPlot <- renderPlotly({
  page_logger("DEBUG", "Rendering manual outlier plot for participant:", input$participant_16, "trial:", input$trialNum_16)
    ...
    base_plot <- tryCatch(
      {
        ...
        preprocessedData <- get_preprocessed_data(current_participant, current_trial)

        plotting$plot_steps(
          plot_data, preprocessedData, current_participant, current_trial,
  ...
  if (nrow(current_data) > 0) {
    p <- p + geom_point(
      data = current_data,
      aes(
        x = .data[[current_xaxis]],
        y = .data[[current_yaxis]],
        text = plot_text
      ),
      alpha = 0, size = 0.1
    ) # Invisible but with hover text
  }
  ...
  if (nrow(hs_outlier_data) > 0) {
    ...
    p <- p + geom_point(
      data = hs_outlier_data,
      aes(
        x = .data[[current_xaxis]],
        y = .data[[current_yaxis]],
        text = plot_text
      ),
      color = "gold", size = 4, shape = 4, alpha = 0.8
    ) # Shape 4 = X, yellow color
  }
  ...
  if (input$show_outliers_on_plot && !is.null(current_step_outliers_data) && nrow(current_step_outliers_data) > 0) {
    ...
    if (nrow(step_marker_data) > 0) {
      ...
      p <- p + geom_text(
        data = step_marker_data,
        aes(
          x = .data[[current_xaxis]],
          y = .data[[current_yaxis]],
          text = plot_text
        ),
        label = "◐", color = "grey60", size = 12, alpha = 0.6
      ) # Unicode left half circle, 2x larger
    }
  }
...
output$heelStrikeOutliersTable <- DT::renderDataTable({
  ...
  if (is.null(result)) {
    ...
    return(DT::datatable(
      empty_data,
      options = list(
        dom = "t", ordering = FALSE, searching = FALSE,
  ...
  if (table_type == "heelstrike") {
    display_data <- data %>%
      dplyr::mutate(
        ...
        Delete = paste0(
          '<button class="btn btn-xs btn-danger" onclick="Shiny.setInputValue(\'',
    ...
    display_data <- data %>%
      dplyr::mutate(
        Time = round(time, 4),
        unique_id = paste0(
          "del_step_", gsub("[^A-Za-z0-9]", "_", participant), "_", trialNum, "_",
          round(time * 1000)
        ),
        Delete = paste0(
          '<button class="btn btn-xs btn-danger" onclick="Shiny.setInputValue(\'',
...
save_outlier_tables <- function(overwrite_mode = FALSE, custom_filenames = NULL) {
  ...
  list(
    hs_path = path_hs, step_path = path_step, hs_filename = fname_hs, step_filename = fname_step,
  ...
  tryCatch(
    {
      ...
      hs_message <- if (hs_changes$added > 0 || hs_changes$removed > 0) {
        paste0(
          "❌ <strong>Heel Strike Outliers:</strong> ",
  ...
  if (!is.null(click_data)) {
    isolate({
      last_click_info(list(
        participant = input$participant_16,
...
output$overwrite_warning <- renderUI({
  if (input$autosave_overwrite) {
    ...
    div(
      style = "font-size: 11px; color: #d63384; margin-top: -5px; margin-bottom: 10px; background-color: #fff3cd; padding: 5px; border-radius: 3px; border-left: 3px solid #ffc107;",
      HTML(paste0(
  ...
  tryCatch(
    {
      ...
      hs_matches <- gait$find_heel_strikes(current_hs_outliers, gait_dt, threshold)
      ...
      hs_impact_details <- if (length(rows_to_remove)) {
        ...
        paste0(
          "❌ HEEL STRIKES TO PERMANENTLY REMOVE: ", length(rows_to_remove), " total\n   Per trial: ",
      ...
      step_matches <- gait$find_heel_strikes(current_step_outliers, gait_dt, threshold)
      ...
      step_impact_details <- if (length(rows_to_mark)) {
        ...
        paste0(
          "◐ STEP OUTLIERS TO MARK: ", length(rows_to_mark), " total\n   Per trial: ",
  ...
  tryCatch(
    {
      ...
      allGaitParams <<- gait$apply_outlier_processing(
        allGaitParams,
        current_hs_outliers,
      ...
      showNotification(
        paste(
          "Outliers applied successfully:", n_outlier_steps, "steps marked as outliers,",
