...
observeEvent(input$apply_to_workspace, {
  tryCatch({
    ...
    if (nrow(hs_outliers_dt) > 0) {
      # Add time window columns for non-equi join
      threshold <- input$outlier_threshold
      hs_outliers_dt[, `:=`(time_min = time - threshold, time_max = time + threshold)]
      
      # Perform a non-equi join to find all matches within the time threshold
      hs_matches <- gait_dt[hs_outliers_dt, 
          on = .(participant, trialNum, time >= time_min, time <= time_max),
          nomatch = 0,
          .(participant, trialNum, time = x.time)] # x.time is the time from gait_dt
      
      if(nrow(hs_matches) > 0){
        # Get indices of rows to be removed by joining back to the original table
        rows_to_remove <- gait_dt[hs_matches, on = .(participant, trialNum, time), which = TRUE]
        
        # Summarize impact
        hs_summary <- hs_matches[, .N, by = .(participant, trialNum)]
        hs_details <- paste0("P", hs_summary$participant, "_T", hs_summary$trialNum, " (", hs_summary$N, ")", collapse = ", ")
        hs_impact_details <- sprintf("❌ HEEL STRIKES TO PERMANENTLY REMOVE: %d total\n   Per trial: %s\n   ⚠️ These will be deleted from the dataset!\n\n",
                                     length(rows_to_remove), hs_details)
      }
    }
    ...
    if (nrow(step_outliers_dt) > 0) {
      # Perform join to find exact matches to mark as step outliers
      # A simple inner join is sufficient here.
      rows_to_mark <- gait_dt[step_outliers_dt, on = .(participant, trialNum, time), nomatch = 0, which = TRUE]
      
      if (length(rows_to_mark) > 0) {
        step_summary <- gait_dt[rows_to_mark, .N, by = .(participant, trialNum)]
        step_details <- paste0("P", step_summary$participant, "_T", step_summary$trialNum, " (", step_summary$N, ")", collapse = ", ")
        step_impact_details <- sprintf("◐ STEP OUTLIERS TO MARK: %d total\n   Per trial: %s\n\n",
                                       length(rows_to_mark), step_details)
      }
    }
...
observeEvent(input$confirm_apply_outliers, {
  tryCatch({
    ...
    allGaitParams <<- apply_outliers_to_gaitparams(
      allGaitParams,
      current_hs_outliers,
