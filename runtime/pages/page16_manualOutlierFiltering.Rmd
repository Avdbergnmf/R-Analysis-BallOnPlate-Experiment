...
observeEvent(input$apply_to_workspace, {
  tryCatch({
    ...
    rows_to_remove <- if (!is.null(hs_matches)) gait_dt[hs_matches,
                          on = .(participant, trialNum, time), which = TRUE] else integer(0)
    ...
    rows_to_mark <- if (!is.null(step_matches)) gait_dt[step_matches,
                       on = .(participant, trialNum, time), which = TRUE] else integer(0)
