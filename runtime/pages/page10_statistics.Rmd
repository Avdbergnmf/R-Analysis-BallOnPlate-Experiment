...
compute_did_contrasts <- function(lmm, correction_method, weights, include_phases) {
  ...
  # Step 2: Within-condition deltas vs baseline
  methods <- build_delta_contrasts_from_reference(emm_pc)
  if (length(methods) == 0) return(data.frame(Message = "Not enough phase levels."))
  
  delta_cont <- emmeans::contrast(emm_pc, method = methods, by = "condition")
  
  # Step 3: Pairwise condition comparisons of those deltas, by phase
  did_res <- emmeans::contrast(delta_cont, method = "pairwise", by = "phase", adjust = correction_method)
  df <- as.data.frame(summary(did_res, infer = c(TRUE, TRUE)))
  
  # Filter to requested phases
  if ("phase" %in% names(df)) {
    names(df)[names(df) == "phase"] <- "Phase"
    df <- df[df$Phase %in% include_phases, , drop = FALSE]
  }
  
  names(df)[names(df) == "contrast"] <- "Contrast"
  
  # Get unadjusted
  did_unadj <- emmeans::contrast(delta_cont, method = "pairwise", by = "phase", adjust = "none")
  df_unadj <- as.data.frame(summary(did_unadj))
  if ("phase" %in% names(df_unadj)) {
    df_unadj <- df_unadj[df_unadj$phase %in% include_phases, , drop = FALSE]
  }
