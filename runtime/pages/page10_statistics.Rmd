...
output$post_hoc_table <- renderTable({
  ...
  if (length(posthoc_results) > 0) {
    ...
    # If paired t-test is enabled, add paired t-test p-values for each comparison
    if (isTRUE(input$paired_ttest) && all(c("Effect", "contrast") %in% names(final_df))) {
      ...
      # Compute paired p-values row-wise
      paired_p <- rep(NA_real_, nrow(final_df))
      ...
      for (i in seq_len(nrow(final_df))) {
        ...
        vars_involved <- unlist(strsplit(indep_key, ":"))
        ...
        for (pid in unique_participants) {
          ...
          for (j in seq_along(vars_involved)) {
            v <- vars_involved[j]
            if (!(v %in% names(dt2))) { rows_left <- FALSE; break }
          ...
          for (j in seq_along(vars_involved)) {
            v <- vars_involved[j]
            if (!(v %in% names(dt2))) { rows_right <- FALSE; break }
        ...
        if (length(left_vals) >= 2 && length(right_vals) >= 2 && length(left_vals) == length(right_vals)) {
          ...
          if (!inherits(tt, "try-error")) {
            paired_p[i] <- tt$p.value
          }
        }
      }
      final_df$paired_t_p <- paired_p
    }
    ...
    if (isTRUE(input$paired_ttest) && ("paired_t_p" %in% names(final_df))) {
      final_df <- set_digits_with_bold_pvalues(final_df, "paired_t_p")
    }
