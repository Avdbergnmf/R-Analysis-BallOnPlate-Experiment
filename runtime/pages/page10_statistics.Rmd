...
selectizeInput("correctionMethod", "Multiple comparison correction (fixed effects)",
  choices = if (exists("general_correction_choices")) general_correction_choices else c("holm", "bonferroni", "BH", "fdr", "none"),
...
conditionalPanel(
  ...
  selectizeInput("posthoc_correction", "Post-hoc correction method",
    choices = c("tukey", "scheffe", "sidak", if (exists("general_correction_choices")) general_correction_choices else character(0)),
...
```{r, context="server"}
# Create page logger for this server context (logging system already loaded globally)
page_logger <- create_module_logger("PAGE10-STATISTICS")
...
get_stats_data <- reactive({
  page_logger("DEBUG", "Getting stats data...")

  # Get the data based on user selection
  data <- if (input$getSummarizedData_stats) {
    page_logger("DEBUG", "Using summarized data (get_mu_dyn_long)")
    get_mu_dyn_long()
  } else {
    page_logger("DEBUG", "Using filtered parameters data")
    filteredParams()
  }

  page_logger("DEBUG", "Raw data dimensions:", nrow(data), "x", ncol(data))

  # Handle empty data case
  if (!stats$validate_stats_data(data)) {
    page_logger("WARN", "Invalid data detected, returning empty dataset")
  ...
  page_logger("DEBUG", "Processed data dimensions:", nrow(dt), "x", ncol(dt))
  dt
})
...
observe({
  data <- get_stats_data()
  page_logger("DEBUG", "Updating reference level selectors...")

  if ("condition" %in% names(data)) {
    available_conditions <- unique(data$condition)
    page_logger("DEBUG", "Available conditions:", paste(available_conditions, collapse = ", "))
    ...
    page_logger("DEBUG", "Setting condition reference level to:", default_ref)
  ...
  if ("phase" %in% names(data)) {
    ...
    page_logger("DEBUG", "Available phases:", paste(available_phases, collapse = ", "))
    ...
    page_logger("DEBUG", "Setting phase reference level to:", default_ref)
...
get_lmer <- reactive({
  ...
  page_logger("INFO", "Building LMM model...")

  data <- get_stats_data()
  page_logger("DEBUG", "Model data dimensions:", nrow(data), "x", ncol(data))
  ...
  if (!is.null(input$refLevelCondition) && nzchar(input$refLevelCondition)) {
    page_logger("DEBUG", "Setting condition reference level to:", input$refLevelCondition)
  ...
  if (!is.null(input$refLevelPhase) && nzchar(input$refLevelPhase)) {
    page_logger("DEBUG", "Setting phase reference level to:", input$refLevelPhase)
  ...
  if (input$depVarTransform != "none") {
    page_logger("DEBUG", "Applying transformation:", input$depVarTransform, "to", input$depVar)
  }
  data <- stats$transform_dependent_variable(data, input$depVar, input$depVarTransform)
  if (is.null(data)) {
    page_logger("ERROR", "Data transformation failed")
    return(NULL)
  }

  doCustom <- input$useCustomModel && !is.null(input$customModel) && nzchar(input$customModel)
  if (doCustom) {
    page_logger("INFO", "Using custom model formula:", input$customModel)
  ...
  page_logger("INFO", "Fitting mixed model...")
  result <- stats$fit_mixed_model(data, formula, input$depVar, input$doCentering, input$doScaling)

  if (is.null(result)) {
    page_logger("ERROR", "Model fitting failed")
  } else {
    page_logger("INFO", "Model fitted successfully")
  }
...
observeEvent(input$ttest_assumption_row, {
  page_logger("INFO", "T-test assumption checking for row:", input$ttest_assumption_row)

  lookup <- ttest_assumption_lookup()
  info <- lookup[[input$ttest_assumption_row]]
  if (is.null(info)) {
    page_logger("ERROR", "Assumption diagnostics unavailable for row:", input$ttest_assumption_row)
  ...
  page_logger("DEBUG", "Showing assumption diagnostics for:", info$effect, "-", info$contrast)
...
output$pca_button_ui <- renderUI({
  page_logger("DEBUG", "Rendering PCA button...")
...
observeEvent(input$show_pca_analysis,
  {
    page_logger("INFO", "PCA button click detected!")
  },
  ignoreInit = TRUE
)
...
observeEvent(input$show_pca_analysis, {
  page_logger("INFO", "PCA button clicked! Starting PCA analysis...")

  data <- get_stats_data()
  page_logger("INFO", "Data retrieved. Dimensions:", nrow(data), "x", ncol(data))

  if (is.null(data) || nrow(data) == 0) {
    page_logger("ERROR", "No data available")
    showNotification("No data available for PCA analysis.", type = "error")
    return()
  }

  # Get available variables
  page_logger("INFO", "Getting available variables...")
  available_vars <- stats$get_pca_variables(data)
  page_logger("INFO", "Available variables:", length(available_vars), "variables found")
  page_logger("DEBUG", "Variables:", paste(available_vars, collapse = ", "))

  if (length(available_vars) < 2) {
    page_logger("ERROR", "Not enough variables (", length(available_vars), ")")
    showNotification("Need at least 2 numeric variables for PCA analysis.", type = "warning")
    return()
  }

  # Show variable selection modal first
  page_logger("INFO", "Showing variable selection modal...")
  showModal(modalDialog(
  ...
  page_logger("INFO", "Modal displayed successfully")
})
...
observeEvent(input$show_pca_analysis, {
  ...
  default_vars <- c(
    ...
    "task.pVelAbsolute_mean", "task.drop_risk_1s_mean", "treadmillSpeed.mean", "task.total_score",
    "complexity.hipPos_power_high", "complexity.pelvisPos_power_mid", "complexity.pelvisPos_power_low"
  )
  ...
  page_logger("INFO", "Set default PCA variables:", length(selected_defaults), "out of", length(default_vars), "requested")
})
...
observeEvent(input$add_vars_by_string, {
  ...
  # Basic input validation to prevent problematic patterns
  if (nchar(input$pca_string) > 200) {
    showNotification("Pattern too long. Please use a shorter search pattern.", type = "warning")
    return()
  }

  # Check for potentially problematic recursive patterns
  if (grepl("(AND.*AND|OR.*OR)", input$pca_string, ignore.case = TRUE)) {
    showNotification("Complex nested patterns detected. Please simplify your search pattern.", type = "warning")
    return()
  }
  ...
  tryCatch(
    {
      ...
      if (is.null(matching_vars)) matching_vars <- character(0)
      new_selected <- unique(c(current_selected, matching_vars))
      updateSelectizeInput(session, "pca_variables", selected = new_selected)
      page_logger("INFO", "Added", length(matching_vars), "variables matching pattern '", input$pca_string, "'")
    },
    error = function(e) {
      page_logger("ERROR", "Error in variable matching:", e$message)
      showNotification(paste("Error matching variables:", e$message), type = "error")
    }
  )
})
  ...
  tryCatch(
    {
      ...
      updateSelectizeInput(session, "pca_variables", selected = new_selected)
      page_logger("INFO", "Removed", length(matching_vars), "variables matching pattern '", input$pca_string, "'")
    },
    error = function(e) {
      page_logger("ERROR", "Error in variable matching:", e$message)
      showNotification(paste("Error matching variables:", e$message), type = "error")
...
observeEvent(input$add_all_vars, {
  ...
  page_logger("INFO", "Added all", length(pca_available_vars()), "variables")
})

# Clear all variables
observeEvent(input$clear_all_vars, {
  page_logger("INFO", "Clear all button clicked")
  updateSelectizeInput(session, "pca_variables", selected = character(0))
  page_logger("INFO", "Cleared all selected variables")
})
...
observeEvent(input$run_pca, {
  page_logger("INFO", "Run PCA button clicked! Starting analysis...")

  data <- get_stats_data()
  page_logger("INFO", "Data retrieved for PCA analysis. Dimensions:", nrow(data), "x", ncol(data))

  # Get selected variables (NULL means use all)
  selected_vars <- input$pca_variables
  if (length(selected_vars) == 0) {
    selected_vars <- NULL
    page_logger("INFO", "No variables selected, will use all numeric variables")
  } else {
    page_logger("INFO", "Selected variables:", length(selected_vars), "variables")
    page_logger("DEBUG", "Selected variables:", paste(selected_vars, collapse = ", "))
  }

  # Run PCA analysis
  page_logger("INFO", "Running PCA analysis...")
  pca_results <- stats$run_pca_analysis(data, selected_vars)

  # Check for errors
  if (!is.null(pca_results$error)) {
    page_logger("ERROR", "PCA analysis failed:", pca_results$error)
  ...
  page_logger("INFO", "PCA analysis completed successfully")
  ...
  showModal(
    modalDialog(
      ...
      tags$div(
        ...
        tags$p("Each point represents an observation projected onto the first two principal components. Use mouse to zoom and pan."),
        renderPlotly({
          pca_results$PCA12_interactive
        }),
        tags$hr(),
...
observeEvent(input$show_did_analysis, {
  req(dep_var_valid())
  page_logger("INFO", "DiD analysis button clicked!")

  lmm <- get_lmer()
  data <- get_stats_data()

  if (is.null(lmm)) {
    page_logger("ERROR", "Model could not be fitted for DiD analysis")
    if (exists("showNotification")) {
  ...
  page_logger("INFO", "Running DiD analysis with correction method:", input$correctionMethod)
...
observeEvent(input$show_lmm_assumptions, {
  req(dep_var_valid())
  page_logger("INFO", "LMM assumption checking button clicked!")
