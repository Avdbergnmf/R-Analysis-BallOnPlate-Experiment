...
textInput("customModel", "Custom Model Formula: dependent_variable ~ ", value = "baseline + condition * phase.x + (1 | participant)")
...
get_stats_data <- reactive({
  ...
  if (!is.null(input$depVar) && input$depVar %in% names(data) &&
    ...
    if (nrow(baseline_rows) > 0) {
      ...
      # Remove baseline rows from the working dataset unless the user is
      # explicitly analysing the baseline variable itself. This ensures that
      # subsequent analyses are performed on non-baseline trials only.
      if (input$depVar != "baseline") {
  ...
  # DEBUG -----------------------------------------------------------------
  # Print the number of observations that will be returned. This will appear
  # in the R console and can help diagnose unexpected filtering behaviour.
  cat("[DEBUG] get_stats_data: returning", nrow(data), "rows\n")
...
output$post_hoc_table <- renderTable({
  ...
  cat("[DEBUG] Available variables in model data:", paste(names(model_data), collapse = ", "), "\n")
  cat("[DEBUG] Input variables:", paste(all_indep_vars, collapse = ", "), "\n")
  ...
  for (indep_var in effects_to_test) {
    if (grepl(":", indep_var)) {
      ...
      # Debug: print what we're passing to emmeans
      cat("[DEBUG] Interaction vars for", indep_var, ":", paste(interaction_vars, collapse = ", "), "\n")
      
      # Validate that we have valid variable names
      if (length(interaction_vars) == 0 || any(interaction_vars == "")) {
        cat("[WARNING] Skipping", indep_var, "- invalid interaction variables\n")
        next
      }
      ...
      # Debug: print what we're passing to emmeans
      cat("[DEBUG] Base var name for", indep_var, ":", base_var_name, "\n")
      ...
      if (is.null(base_var_name) || base_var_name == "" || !(base_var_name %in% names(model_data))) {
        cat("[WARNING] Skipping", indep_var, "- invalid base variable name:", base_var_name, "\n")
        next
      }
  ...
  if (length(posthoc_results) > 0) {
    ...
    # Deduplicate effects by creating a mapping from original to cleaned names
    effect_mapping <- data.frame(
      original = names(posthoc_results),
      cleaned = pretty_effect_names,
      stringsAsFactors = FALSE
    )
    
    # Group by cleaned effect names to deduplicate
    unique_effects <- unique(effect_mapping$cleaned)
    final_results <- list()
    
    for (effect in unique_effects) {
      # Get all original effects that map to this cleaned effect
      original_effects <- effect_mapping$original[effect_mapping$cleaned == effect]
      
      # Combine results from all original effects
      combined_results <- do.call(rbind, posthoc_results[original_effects])
      final_results[[effect]] <- combined_results
    }
