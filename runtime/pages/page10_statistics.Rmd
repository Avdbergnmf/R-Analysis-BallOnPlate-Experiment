...
get_lmer <- reactive({
  ...
  if (!is.null(input$refLevelCondition) && nzchar(input$refLevelCondition)) {
    data <- apply_custom_reference_level(data, "condition", input$refLevelCondition)
  }
  if (!is.null(input$refLevelPhase) && nzchar(input$refLevelPhase)) {
    data <- apply_custom_reference_level(data, "phase", input$refLevelPhase)
  }

  # Apply transformation to dependent variable if selected
  data <- apply_depvar_transform(data, input$depVar, input$depVarTransform)
  if (is.null(data)) {
    return(NULL)
  }
...
# General correction method choices (for other parts of the UI)
...
observeEvent(input$show_did_analysis, {
  ...
  # Compute DiD analysis (uses Kenward-Roger df and Holm correction)
  did_results <- compute_did_analysis(lmm, data)
    ...
    modalDialog(
      ...
      tags$div(
        ...
        tags$ul(
          ...
          tags$li(tags$strong("Multiple Comparison Correction:"), " Holm step-down procedure (controls family-wise error rate while being more powerful than Bonferroni)"),
