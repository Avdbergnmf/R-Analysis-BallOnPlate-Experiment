...
checkboxInput("paired_ttest", "Use Paired t-test instead of emmeans", value = TRUE)
```
...
create_dynamic_variable_observer(
  ...
  default_choice = "task.dist_to_escape_mean", # "stepWidths.sd",
...
output$post_hoc_table <- renderTable(
  {
    ...
    # Prepare analysis
    lmm_posthoc <- prepare_posthoc_analysis(lmm, data, input$indepInterVars, input$indepVars, input$force_posthoc)
...
output$ttest_assumption_details <- renderUI({
  ...
  tagList(
    tags$div(
      ...
      tags$p(tags$strong("Levels compared:"),
             sprintf("%s  vs  %s",
...
output$ttest_assumption_hist <- renderPlot({
  ...
  hist(diffs, main = "Histogram of Paired Differences",
...
output$ttest_assumption_box <- renderPlot({
  ...
  boxplot(diffs, main = "Boxplot of Differences", 
          ylab = "Differences (Right - Left)",
          col = "lightblue", border = "darkblue")
  abline(h = 0, col = "red", lty = 2)
  # Add mean line
  abline(h = mean(diffs), col = "green", lwd = 2)
  legend("topright", c("Zero line", "Mean"), col = c("red", "green"), 
