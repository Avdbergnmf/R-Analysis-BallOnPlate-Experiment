  ...
  if (!is.null(input$filterCondition) && length(input$filterCondition) > 0) {
    ...
    for (participant in input$filterParticipants) {
      ...
      if (participant_condition %in% input$filterCondition) {
        ...
        filter_logger("DEBUG", "Participant", participant, "(condition", participant_condition, ") matches filter")
      } else {
        filter_logger("DEBUG", "Participant", participant, "(condition", participant_condition, ") does NOT match filter")
      }
    }
