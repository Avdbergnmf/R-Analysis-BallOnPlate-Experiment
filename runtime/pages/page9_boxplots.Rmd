...
bsTooltip("plotVarTransform",
  "Apply transformation to the plotted variable. Log10 transformation can help with skewed data.",
  placement = "right",
  trigger = "hover"
)
```
...
generate_boxplot <- function() {
  tryCatch({
    ...
    if (input$plotVarTransform == "log10") {
      # Check if all values are positive for log10 transformation
      plot_var_values <- mu[[input$plotVar]]
      if (any(plot_var_values <= 0, na.rm = TRUE)) {
        stop("Cannot apply log10 transformation: Some values are <= 0. Consider adding a constant or using a different transformation.")
      }
      # Apply log10 transformation directly to the column in mu
      mu[[input$plotVar]] <- log10(mu[[input$plotVar]])
    }
