...
# Render the static scatterplot (only when interactive is not selected)
output$scatter_plot <- renderSVG({ 
  # Only render when interactive plot is not selected
  if (!input$useInteractivePlot) {
    ...
    if (is.null(data) || nrow(data) == 0) {
      return(plotting$create_error_plot("No data available for plotting"))
    }
    
    if (is.null(x_var) || is.null(y_var) || is.null(group_var)) {
      return(plotting$create_error_plot("Please select variables for plotting"))
    }
...
# Render the interactive scatterplot (only when interactive is selected)
output$scatter_plot_interactive <- renderPlotly({
  # Only render when interactive plot is selected
  if (input$useInteractivePlot) {
    data <- get_scatter_data()
    group_var <- input$groupScatter
    x_var <- input$xscatter
    ...
    # Create the base ggplot (using interactive version without marginal plots)
    p <- plotting$make_scatter_plot_interactive(data, group_var, x_var, y_var, show_legend = TRUE, baseSize = input$baseSize)
