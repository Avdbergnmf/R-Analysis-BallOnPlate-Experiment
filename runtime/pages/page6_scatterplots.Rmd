...
page_logger <- create_module_logger("PAGE6-SCATTERPLOTS")
...
output$scatter_plot <- renderSVG({
  reactive({
    page_logger("DEBUG", "Rendering static scatterplot")
    plot_data <- get_plot_data()

    # Check for errors
    if (!is.null(plot_data$error)) {
      page_logger("WARN", "Error in plot data:", plot_data$error)
...
output$scatter_plot_interactive <- renderPlotly({
  page_logger("DEBUG", "Rendering interactive scatterplot")
  plot_data <- get_plot_data()
