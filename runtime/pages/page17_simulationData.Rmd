...
output$sim_trajPlot <- renderPlotly({
  req(input$sim_participant, input$sim_trialNum)
  data <- get_simulation_data(input$sim_participant, input$sim_trialNum)
...
output$sim_tsPlot <- renderPlotly({
  req(input$sim_participant, input$sim_trialNum, input$sim_vars)
  data <- get_simulation_data(input$sim_participant, input$sim_trialNum)
  if (is.null(data) || nrow(data) == 0) return(NULL)
  
  plot_time_series(data, input$sim_vars)
})
``` 