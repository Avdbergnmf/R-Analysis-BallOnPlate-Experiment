...
sim_data <- reactive({
  ...
  page_logger("DEBUG", "Loading simulation data for participant:", participant, "trial:", trial)

  if (!exists("simulation", envir = .GlobalEnv)) {
    page_logger("ERROR", "Simulation feature module is not available")
  ...
  needs_override <- !is.null(standardized_condition) || !is.null(standardized_phase)

  model_available <- base::exists("GLOBAL_RISK_MODEL", envir = .GlobalEnv) &&
    !is.null(base::get("GLOBAL_RISK_MODEL", envir = .GlobalEnv))
  require_direct_model <- isTRUE(input$allow_direct_model) || needs_override
  if (isTRUE(input$enable_risk_predictions) && require_direct_model && !model_available && !risk_analysis_state$model_modal_shown) {
  ...
  allow_direct_model <- model_available && (isTRUE(input$allow_direct_model) || needs_override)
  force_direct_model <- isTRUE(input$enable_risk_predictions) && model_available && (isTRUE(input$allow_direct_model) || needs_override)

  sim_data <- simulation$add_risk_predictions(sim_data,
    enable_risk = input$enable_risk_predictions,
    allow_direct_model = allow_direct_model,
...
output$sim_tsPlot <- renderPlotly({
  page_logger("DEBUG", "Rendering simulation time series plot")
  data <- sim_data()
  vars <- input$sim_vars
  if (is.null(data) || nrow(data) == 0 || length(vars) == 0) {
    page_logger("WARN", "No data or variables selected for simulation plot")
    return(plotly::plot_ly() %>% layout(title = "No data/variables selected"))
  }

  if (!exists("simulation", envir = .GlobalEnv)) {
    page_logger("ERROR", "Simulation module not available for plotting")
