...
```{r, context="server"}
risk_analysis_state <- reactiveValues(pending = NULL)
...
observeEvent(input$load_risk_model, {
  ...
  if (!exists("simulation", envir = .GlobalEnv)) {
    output$risk_model_status <- renderText("Simulation feature module is not available")
    return()
  }
  ...
  tryCatch(
    {
      ...
      if (!is.null(model)) {
        ...
        output$risk_model_status <- renderText(
          sprintf(
            "Ã¢Å“â€œ Risk model loaded successfully: %s (class: %s)",
            input$risk_model_file, class(model)[1]
          )
        )
      } else {
        output$risk_model_status <- renderText("Ã¢Å“â€” Failed to load risk model")
      }
    },
    error = function(e) {
      output$risk_model_status <- renderText(paste("Ã¢Å“â€” Error loading risk model:", e$message))
    }
  )
})
  ...
  tryCatch(
    {
      ...
      if (!is.null(model)) {
        ...
        output$risk_model_status <- renderText(
          sprintf(
            "âœ“ Risk model trained and loaded successfully! Used %d participants, %d trials, %d observations. Model saved to %s and loaded into global workspace. Use 'Predict' to refresh per-sample predictions before running risk analysis.",
  ...
  tryCatch(
    {
      ...
      showModal(modalDialog(
        title = "Ã¢Å“â€œ Model Applied and Predictions Computed Successfully!",
        paste0(
...
observeEvent(input$perform_risk_analysis, {
  ...
  if (!file.exists(model_path)) {
    output$risk_model_status <- renderText("Selected risk model file does not exist")
    return()
  }

  if (!file.exists(hazard_samples_path)) {
    output$risk_model_status <- renderText("Selected hazard samples file does not exist")
    return()
  }

  if (needs_prediction_refresh(hazard_samples_path)) {
    output$risk_model_status <- renderText("Hazard samples were updated after predictions were generated. Review and confirm before continuing.")
    ...
    showModal(modalDialog(
      ...
      paste0(
        "The base hazard samples were updated after ", basename(risk_hazard_samples_preds_path),
        " was generated. Do you want to proceed with the older predictions?"
      ),
      easyClose = FALSE,
      footer = tagList(
        modalButton("Cancel"),
