  ...
  tryCatch(
    {
      ...
      results <- compute_pooled_standardized_risks(
        model = model,
        hazard_samples = hazard_samples,
        save_csv_path = risk_standardized_path # Save to the same CSV as training
      )

      # Store results globally for potential use
      GLOBAL_HAZARD_PREDICTIONS <<- list(
        hazard_samples = hazard_samples,
        standardized_risks = results$standardized_risks,
      ...
      # Always computed fresh predictions
      prediction_method <- "Computed fresh predictions and saved annotated version"

      # Create popup message with plot
      showModal(modalDialog(
        title = "✓ Pooled Standardized Risks Computed Successfully!",
        paste0(
          "Results: ", n_combinations, " condition × phase combinations from ",
          n_samples, " samples (", n_participants, " participants)\n\n",
          "Method: ", prediction_method, "\n\n",
          "Detailed results printed to console and saved to files.\n",
          "Analysis results stored in GLOBAL_HAZARD_PREDICTIONS"
        ),
