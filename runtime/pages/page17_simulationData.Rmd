  ...
  tryCatch(
    {
      ...
      if (!is.null(model)) {
        ...
        if (is.list(model) && "model" %in% names(model)) {
          # Standardized training was enabled, model is a list
          actual_model <- model$model
          standardized_means <- model$standardized_means
          
          # Load the actual model into global workspace
          ...
          output$risk_model_status <- renderText(
            sprintf(
              "✓ Risk model trained with standardized analysis and loaded successfully! Used %d participants, %d trials, %d observations. Model saved to %s and loaded into global workspace.",
          ...
          # Standard training, model is the actual model object
          GLOBAL_RISK_MODEL <<- model
