...
page_logger <- create_module_logger("PAGE18-POWER-SPECTRUM")
page_logger("DEBUG", "Initializing power spectrum page options")
...
# Create power spectrum data cache manager (with parallel processing disabled by default)
ps_cache_manager <- create_shiny_cache_manager("power_spectrum", use_parallel = FALSE)

# Create cached power spectrum data reactive using the new system
cachedPowerSpectrumData <- create_shiny_cached_data_reactive(
...
observe({
  if (!is.null(input$psTracker)) {
    ...
    choices <- get_tracker_options(input$psTracker, 7)
...
observeEvent(input$psUpdateTrackerData, {
  page_logger("INFO", "Power spectrum data update requested")
...
output$powerSpectrumPlot <- renderPlot(
  {
    page_logger("DEBUG", "Rendering power spectrum plot - enabled:", input$psEnablePlotting)

    # Check if plotting is enabled
    if (!input$psEnablePlotting) {
      page_logger("DEBUG", "Power spectrum plotting disabled")
      return(ggplot() +
        annotate("text",
          x = 0.5, y = 0.5,
    ...
    tryCatch(
      {
        ...
        psd$plot_psd(
          ps_data = ps_data,
          plot_type = if (input$psStack) "area" else "line",
