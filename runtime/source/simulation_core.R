...
hampel_filter_causal <- function(x, k = 3, t0 = 3) {
  n  <- length(x)
  xf <- x
  for (i in seq_len(n)) {
    lo  <- max(1, i - k)      # past-only window
    win <- xf[lo:i]
    med <- median(win)
    s   <- 1.4826 * median(abs(win - med))   # MAD → σ̂
    if (is.finite(s) && s > 0 &&
        abs(xf[i] - med) > t0 * s)
      xf[i] <- med            # replace spike
  }
  xf
}
...
kalman_cv_filter <- function(pos,
                             ...
                             sigma_m  = 0.002) {    # meas. noise     (m)
  n <- length(pos)
  if (n == 0L) return(list(p = numeric(0), v = numeric(0)))
  
  ## handle dt vector vs scalar -------------------------------------------
  if (length(dt) == 1L)           dt <- rep(dt, n)
...
patch_gaps <- function(time, pos, tol = 1.5) {
  ...
  if (!length(gap)) return(list(time=time, pos=pos))
