...
sampen <- function(x, m = 2, r = 0.2 * sd(x)) {
  ...
  if (matches_m == 0 || matches_m1 == 0) return(NA)
...
mse <- function(x, max.scale = 30, m = 2, r = 0.2 * sd(x)) {
  if (length(x) < 30) return(list(mse = NA, complexity = NA))
...
dfa_alpha <- function(x, min.box = 4, max.frac = 0.25, n.points = 20) {
  ...
  if (sum(valid_indices) < 3) return(NA)
...
lyapunov <- function(x, emb.dim = 5, delay = 1, fit.range = c(0, 1)) {
  ...
  if (length(valid_divergences) < 10) return(NA)
  ...
  if (end_idx - start_idx < 3) return(NA)
...
poincare_sd <- function(x) {
  if (length(x) < 3) return(list(sd1 = NA, sd2 = NA, sd1_sd2 = NA))
