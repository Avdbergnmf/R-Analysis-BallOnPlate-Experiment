...
data_loaders <<- list(
  ...
  "power_spectrum" = function(participant, trial) {
...
#' Define data validation functions for different data types
#' Each function should take (participant, trial) and return TRUE/FALSE
data_validators <<- list(
  "simulation" = function(participant, trial) {
  ...
  # Add more data validators here as needed
  # "gait" = function(participant, trial) {
  #   return(has_gait_data(participant, trial))
  # },
  # 
  # "questionnaire" = function(participant, trial) {
  #   return(has_questionnaire_data(participant, trial))
  # }
)
...
is_data_type_supported <- function(data_type) {
  return(data_type %in% names(data_loaders) && data_type %in% names(data_validators))
}
...
#' Get data validator for a specific data type
#' @param data_type The data type
#' @return The validator function or NULL if not found
get_data_validator <- function(data_type) {
  return(data_validators[[data_type]])
}
...
#' @param validator_function Function that validates if data exists for a participant-trial combination
add_data_type <- function(data_type, loader_function, validator_function) {
  if (!is.function(loader_function)) {
    stop("loader_function must be a function")
  }
  if (!is.function(validator_function)) {
    stop("validator_function must be a function")
  }
  
  data_loaders[[data_type]] <<- loader_function
  data_validators[[data_type]] <<- validator_function
...
remove_data_type <- function(data_type) {
  ...
  if (data_type %in% names(data_validators)) {
    data_validators[[data_type]] <<- NULL
  }
