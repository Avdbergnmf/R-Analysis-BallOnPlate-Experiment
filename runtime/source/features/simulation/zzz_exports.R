#' Simulation Module Export Management
#'
#' Collects the functions defined across the simulation feature files and
#' exposes them through the `simulation` object returned by the module loader.

simulation <- local({
  feature_env <- parent.env(environment())

  # Functions injected by the loader that we don't want to export explicitly.
  reserved_names <- c("create_logger", "create_module_logger", "simulation")
