...
## Get Results - Load data BEFORE feature modules
...
## Summary tables
# Note: summarize_gaitparams.R functions are now integrated into the gait feature module
...
# Load all feature modules AFTER data is available
cat("=== LOADING FEATURE MODULES ===\n")
feature_names <- c("gait", "psd", "stats", "outliers")
for (feature_name in feature_names) {
  if (!exists(feature_name, envir = .GlobalEnv)) {
    cat(sprintf("Loading %s feature module...\n", feature_name))
    assign(feature_name, load_feature(feature_name), envir = .GlobalEnv)
    cat(sprintf("✓ %s feature module loaded\n", feature_name))
  } else {
    cat(sprintf("✓ %s feature module already loaded\n", feature_name))
  }
}
cat("=== ALL FEATURE MODULES LOADED ===\n")

# Load remaining data objects
cat("=== LOADING REMAINING DATA OBJECTS ===\n")
allTaskMetrics <<- load_or_calculate(taskMetricsFile, get_all_task_metrics, parallel = FALSE)
allComplexityMetrics <<- load_or_calculate(complexityMetricsFile, get_all_complexity_metrics, stop_cluster = TRUE)
cat("=== ALL DATA OBJECTS LOADED ===\n")

# Task and complexity metrics are now loaded above with other data objects
...
source("source/utils/get_filtered_data.R", local = TRUE)
```
