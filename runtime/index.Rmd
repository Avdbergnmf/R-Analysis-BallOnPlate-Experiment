...
if (file.exists(risk_model_path)) {
  GLOBAL_RISK_MODEL <<- load_risk_model(risk_model_path)
  GLOBAL_RISK_TAU <<- attr(GLOBAL_RISK_MODEL, "tau")
  cat("Risk model loaded from:", risk_model_path, "\n")
  if (!is.null(GLOBAL_RISK_TAU)) {
    cat("Risk model tau:", GLOBAL_RISK_TAU, "seconds\n")
  } else {
    cat("No tau found in risk model\n")
  }
} else {
  cat("No risk model found at:", risk_model_path, "- use dashboard to load one\n")
}

# Load hazard samples with predictions into global workspace (only if it exists)
if (file.exists(risk_hazard_samples_preds_path)) {
  GLOBAL_HAZARD_SAMPLES_PREDS <<- readRDS(risk_hazard_samples_preds_path)
  cat("Hazard samples with predictions loaded from:", risk_hazard_samples_preds_path, "\n")
  cat("Loaded", nrow(GLOBAL_HAZARD_SAMPLES_PREDS), "hazard prediction rows\n")
} else {
  cat("No hazard samples with predictions found at:", risk_hazard_samples_preds_path, "\n")
}
